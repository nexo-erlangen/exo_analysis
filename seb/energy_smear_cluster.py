...
def fillEnergy(t, res, bins, hRange, art='ss', MC=True):
    ...
    for i in range( N ):
        ...
        # - Test start -
        # Get only events with two charge cluster
        if es.multiplicity != 2:
            continue

        # Check if two energies are present
        if len(np.array(es.cluster_energy)) != 2:
            continue

        # Cut on full absorption peak
        if MC:
            resolution = [0.714076, 21.9255, 0.]
            en = getNewEnergy(resolution, es.energy_mc)[0]
        else:
            if art == 'ss':
                en = es.energy_ss
            elif art == 'ms':
        ...
        if not MC:
            ...
            # E = [getNewEnergy(res, c_en)[0] * energyCorrMC for c_en in np.array(es.cluster_energy)]
