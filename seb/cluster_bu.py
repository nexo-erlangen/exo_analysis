...
def comptonScattering(d, l=10, rLim=None):
    ...
    for idx, entry in d.iteritems():
        ...
        for val in entry:
            ...
            energyTotalList.append( energyFirst + energySecond )
        ...
        electronList += zip([z] * len(electronAngleList), [phi] * len(electronAngleList), thetaListFilt, energyListFilt, electronAngleList, interactionEnergyList, electronAngleReList, vecGamma1List, vecGamma2AngleList, xList, yList)
...
def getClusterPos(tree, FV, art=None):
...
def getEnergyInfo(d, dMC, smooth=False, src='ThS5'):
    ...
    angleBins = 6
    mergeListList = mergeBoxData(d, boxList, angleBins, False)
    mergeListListMC = mergeBoxData(dMC, boxList, angleBins, True)
    ...
    plotXY([angleList] * 2, [meanListAll, meanListMCAll], r'Electron angle $\theta_e$ [$^\circ$]', r'$\frac{\overline{E_T}}{\overline{{E_T}_\textrm{mean}}}$', ppRes, smooth, 'All scattering angles', show=False)
...
def mergeBoxData(d, boxList, binN=12, MC=False):
