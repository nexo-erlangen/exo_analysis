...
def main():
	...
	for startX in np.linspace(0., 0., 1):
		for startY in np.linspace(0.17, REFLECTORINNERRAD - 0.0005, Nr):
			for startZ in np.linspace(0.1, -0.1, Nz):
				if np.sqrt(startX**2 + startY**2) >= REFLECTORINNERRAD:
					continue

                                global par
                                if startZ > 0:
                                    import artificialField_paramEXOTop as par
                                else:
                                    import artificialField_paramEXOBottom as par
